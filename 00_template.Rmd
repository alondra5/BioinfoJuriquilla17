...
The concentrations of the drug are 8 for CCLE and 9 GDSC.
...
ggplot( pharmacoData, aes( log2(viability) ) ) +
    geom_histogram(fill = "lightgreen", colour="purple") +
    facet_wrap(~study)
```
As we can see in the histograms of each experiment (CCLE and GDSC), drug viabilities
...
```
It suppose that we must obtained values between 0 and 100 (%), but that did not happen. There are 23 values that are 
